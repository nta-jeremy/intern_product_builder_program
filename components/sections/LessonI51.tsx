...
const PART_META = [
  { n: "01", short: "Kiến trúc = trade-off & Pattern", title: "Kiến trúc là chuỗi trade-off & Lựa chọn Pattern", time: "~16 phút", c: "var(--iris)", cDeep: "var(--iris-deep)", tint: "var(--iris-tint)" },
...
const PARTS = [
  { ...PART_META[0], desc: "Kiến trúc = chuỗi quyết định trade-off (không phải sơ đồ đẹp), quy tắc vàng nêu yếu tố phải đánh đổi, 6 thành phần của giải pháp production, chọn pattern Prompt / RAG / Agent theo bài toán, single vs multi-agent.", tags: ["Trade-off", "6 thành phần", "Prompt / RAG / Agent", "Single vs Multi-agent"] },
  { ...PART_META[1], desc: "Ba trục đánh đổi latency–cost–accuracy thường xung đột, token cost là ràng buộc thiết kế từ đầu, sai lầm \"chạy tốt khi demo\" & tính tải thực, ba kỹ thuật tối ưu cache / trim / routing.", tags: ["Latency–Cost–Accuracy", "Token cost", "Tính tải thực", "Cache · Trim · Routing"] },
  { ...PART_META[2], desc: "Monitoring song hành cùng kiến trúc & ba câu hỏi (đo gì / log gì không PII / cảnh báo khi nào), eval chống hồi quy, harness chạy & chấm tự động, gói lại thành tài liệu kiến trúc Capstone.", tags: ["Monitoring · 3 câu hỏi", "Eval & hồi quy", "Harness", "Tài liệu kiến trúc"] },
];

const OBJECTIVES = [
  "Giải thích kiến trúc = chuỗi quyết định đánh đổi; mô tả 6 thành phần của một giải pháp AI production.",
  "Lựa chọn mẫu kiến trúc (Prompt đơn / RAG / Agent; single vs multi-agent) dựa trên bài toán thực tế, không chạy theo trend.",
  "Cân bằng latency – cost – accuracy; áp dụng cache, context trimming và model routing để tối ưu.",
  "Thiết kế monitoring đi cùng kiến trúc: đo gì / log gì (không PII) / cảnh báo khi nào.",
  "Giải thích quy trình eval & harness: kiểm tra thay đổi có gây lỗi hồi quy hay không.",
];
...
const EXAM: ExamQ[] = [
  { part: A, q: "\"Kiến trúc giải pháp AI\" đúng nghĩa là gì?", opts: ["Một sơ đồ nhiều hộp và mũi tên cho đẹp", "Một bản vẽ trang trí", "Một chuỗi quyết định đánh đổi (trade-off), mỗi lựa chọn đều đi kèm với chi phí phải đánh đổi", "Một danh sách tính năng"], correct: 2, why: "Kiến trúc là chuỗi quyết định đánh đổi (trade-off), mỗi lựa chọn đều đi kèm cái giá phải trả. (File 1)" },
  { part: A, q: "\"Quy tắc vàng\" khi ra quyết định kiến trúc là gì?", opts: ["Chọn công nghệ mới nhất", "Vẽ sơ đồ càng chi tiết càng tốt", "Luôn dùng agent", "Với mọi quyết định, nêu rõ \"yếu tố phải đánh đổi\" (chi phí / độ trễ / độ tin cậy / khả năng giám sát)"], correct: 3, why: "Quy tắc vàng: nêu rõ \"yếu tố phải đánh đổi\" cho mọi quyết định kiến trúc. (File 1)" },
  { part: A, q: "Một giải pháp AI production có 6 thành phần; thành phần \"dễ bị bỏ quên nhất\" là?", opts: ["Monitoring (Giám sát)", "Input", "Output", "Pre-processing (Tiền xử lý)"], correct: 0, why: "Giám sát (Monitoring) là thành phần dễ bị bỏ quên nhất. (File 1)" },
  { part: A, q: "Pattern \"Prompt đơn\" phù hợp khi nào?", opts: ["Cần kiến thức nội bộ cập nhật", "Tác vụ rõ ràng, chỉ cần khả năng ngôn ngữ / lập luận (viết, phân loại, tóm tắt)", "Cần nhiều bước & nhiều công cụ", "Cần tra cứu tài liệu chính sách"], correct: 1, why: "Prompt đơn: tác vụ rõ, chỉ cần ngôn ngữ / lập luận. (File 1)" },
  { part: A, q: "Pattern \"RAG\" phù hợp khi nào?", opts: ["Tác vụ chỉ cần viết văn", "Khi không có dữ liệu nào", "Cần kiến thức ngoài / nội bộ, cập nhật (chính sách, kho sản phẩm)", "Chỉ cần đổi màu giao diện"], correct: 2, why: "RAG: cần kiến thức ngoài / nội bộ, liên tục cập nhật. (File 1)" },
  { part: A, q: "Pattern \"Agent\" có đặc điểm gì cần lưu ý?", opts: ["Rẻ nhất, đơn giản nhất", "Không cần công cụ", "Luôn nên dùng cho mọi bài toán", "Khả năng xử lý mạnh nhất nhưng chi phí cao và khó kiểm soát — chỉ nên dùng khi thực sự cần thiết"], correct: 3, why: "Agent mạnh nhất nhưng chi phí cao và khó kiểm soát — chỉ dùng khi thực sự cần. (File 1)" },
  { part: A, q: "Ở cấp độ L2, bạn nên dùng loại agent nào?", opts: ["Single-agent (một agent vận hành theo quy trình có điểm kiểm soát - checkpoint)", "Multi-agent phức tạp cho mọi bài toán", "Không dùng pattern nào", "Càng nhiều agent càng tốt"], correct: 0, why: "L2 dùng single-agent (có checkpoint); multi-agent là từ L3 trở lên. (File 1)" },
  { part: A, q: "Nên chọn mẫu kiến trúc (Prompt / RAG / Agent) dựa trên điều gì?", opts: ["Theo trend / đối thủ", "Theo bài toán (nhu cầu thực tế của tác vụ)", "Theo cái nghe \"xịn\" nhất", "Theo cái AI gợi ý đầu tiên"], correct: 1, why: "Chọn mẫu kiến trúc theo bài toán thực tế, không chạy theo xu hướng. (File 1)" },
  { part: B, q: "Ba trục trade-off kiến trúc chính là gì?", opts: ["Token · Prompt · Schema", "Problem · User · Metric", "Latency · Cost · Accuracy", "Git · Test · Deploy"], correct: 2, why: "Ba trục đánh đổi: Latency (độ trễ) · Cost (chi phí) · Accuracy (độ chính xác). (File 2)" },
  { part: B, q: "Nhận định nào ĐÚNG về latency / cost / accuracy?", opts: ["Luôn có một cấu hình tốt nhất tuyệt đối", "Ba trục độc lập, không ảnh hưởng nhau", "Tăng accuracy luôn miễn phí", "Chúng thường xung đột lẫn nhau; chỉ có cấu hình phù hợp với ràng buộc của sản phẩm và nêu rõ yếu tố phải đánh đổi"], correct: 3, why: "Ba yếu tố thường xung đột; chỉ có cấu hình phù hợp ràng buộc + nêu rõ yếu tố phải đánh đổi. (File 2)" },
  { part: B, q: "Vì sao \"token cost là quyết định thiết kế\"?", opts: ["Ở quy mô lớn, chi phí token tăng lên rất nhanh (VD: 100.000 lượt × 500 token) → phải tính toán từ đầu, không để tính sau", "Vì token miễn phí", "Vì token không liên quan chi phí", "Vì chỉ demo mới tốn token"], correct: 0, why: "Ở quy mô lớn chi phí token tăng rất nhanh → là quyết định thiết kế cốt lõi, tính từ đầu. (File 2, nối I1.1)" },
