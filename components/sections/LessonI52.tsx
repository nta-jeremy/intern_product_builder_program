...
const PARTS = [
  { ...PART_META[0], desc: "Guardrail bảo vệ cả hai đầu (input / output) + fallback; chọn rule-based hay model-based theo mức độ rõ ràng của tiêu chí; prompt injection tấn công qua dữ liệu (đặc biệt nguy hiểm và âm thầm qua RAG) cùng 3 lớp phòng thủ chiều sâu.", tags: ["Guardrail 2 đầu", "Rule vs Model", "Prompt injection", "3 lớp phòng thủ"] },
  { ...PART_META[1], desc: "Bộ kịch bản đánh giá (eval set) đủ 3 nhóm (thông thường / biên / tấn công đối nghịch) để phát hiện và ngăn ngừa lỗi hồi quy; khử thông tin cá nhân (PII) tự động ở 3 vị trí bắt buộc theo Luật số 91/2025/QH15; lập bảng theo dõi rủi ro (risk register) phân công cụ thể người chịu trách nhiệm.", tags: ["Eval 3 nhóm", "Hồi quy", "PII 3 nơi", "Risk register"] },
  { ...PART_META[2], desc: "Docker đóng gói ứng dụng và cấu hình môi trường (phân biệt Image và Container); hướng dẫn đóng gói an toàn (tuyệt đối không kèm secret / PII); thiết lập giám sát chi phí token và checklist hoàn thiện để đưa vào vận hành (production readiness).", tags: ["Docker", "Image vs Container", "Không secret / PII", "Production readiness"] },
];

const OBJECTIVES = [
  "Thiết kế được rào chắn (guardrail) đầu vào & đầu ra; phân biệt được khi nào dùng luật cứng (rule-based) hay mô hình AI (model-based).",
  "Nhận diện và phòng tránh được prompt injection (kể cả gián tiếp qua RAG) bằng 3 lớp phòng thủ chiều sâu.",
  "Tự động hóa được việc khử thông tin cá nhân (PII) ở quy mô lớn tại 3 vị trí bắt buộc; tuân thủ Luật số 91/2025/QH15.",
  "Xây dựng được bộ kịch bản đánh giá (eval set) đo lường độ chính xác (accuracy) & phát hiện lỗi hồi quy; lập được risk register kèm người chịu trách nhiệm.",
  "Đóng gói được giải pháp bằng Docker ở mức cơ bản và thiết lập giám sát chi phí token khi vận hành.",
];
...
const EXAM: ExamQ[] = [
  { part: A, q: "Rào chắn an toàn (Guardrail) nên được thiết lập ở những vị trí nào?", opts: ["Chỉ ở đầu vào (input)", "Ở cả hai đầu: đầu vào (chặn yêu cầu ngoài phạm vi / độc hại) và đầu ra (chặn phản hồi sai định dạng / không an toàn / lộ dữ liệu)", "Chỉ ở đầu ra (output)", "Không cần rào chắn nếu mô hình AI hoạt động ổn định"], correct: 1, why: "Guardrail cần đặt ở cả hai đầu: input (chặn yêu cầu ngoài phạm vi / độc hại) và output (chặn sai định dạng, thiếu an toàn, rò rỉ dữ liệu). (Phần 1)" },
  { part: A, q: "Rào chắn đầu ra (output guardrail) có nhiệm vụ ngăn chặn những yếu tố nào?", opts: ["Câu hỏi nằm ngoài phạm vi ở đầu vào", "Các chỉ thị có ý đồ độc hại ở đầu vào", "Kết quả đầu ra sai định dạng, thiếu an toàn hoặc rò rỉ PII trước khi gửi tới người dùng", "Vượt giới hạn số lượng token cho phép"], correct: 2, why: "Rào chắn đầu ra ngăn kết quả sai định dạng, thiếu an toàn hoặc lộ thông tin nhạy cảm trước khi hiển thị. (Phần 1)" },
  { part: A, q: "Khi nào nên ưu tiên luật cứng (Rule-based) cho rào chắn thay vì mô hình AI (Model-based)?", opts: ["Khi tiêu chí mơ hồ, cần hiểu sâu ngữ nghĩa", "Khi cần phát hiện nội dung độc hại diễn đạt tinh vi", "Luôn dùng mô hình AI để đảm bảo an toàn tối đa", "Khi tiêu chí hoàn toàn rõ ràng, xác định tường minh (khớp JSON, kiểm từ khóa cấm, đối chiếu mẫu email / SĐT)"], correct: 3, why: "Dùng rule khi tiêu chí rõ ràng, kiểm tra tường minh được (định dạng, nhãn có sẵn, regex email / SĐT). (Phần 1)" },
  { part: A, q: "Khái niệm tấn công chỉ thị \"Prompt injection\" là gì?", opts: ["Cài cắm câu lệnh ẩn vào dữ liệu đầu vào (câu hỏi người dùng hoặc tài liệu trong kho RAG) nhằm thay đổi hành vi mặc định của mô hình", "Tối ưu cấu trúc prompt giúp mô hình trả lời tốt hơn", "Kỹ thuật tăng tốc độ xử lý của mô hình", "Một câu lệnh quản lý phiên bản trong Git"], correct: 0, why: "Prompt injection = đưa lệnh ẩn vào dữ liệu đầu vào để chiếm quyền / đổi chỉ thị gốc của mô hình. (Phần 1)" },
  { part: A, q: "Vì sao tấn công prompt injection qua hệ thống RAG lại nguy hiểm hơn?", opts: ["Vì RAG làm giảm tốc độ xử lý", "Vì mô hình nạp tài liệu truy hồi làm ngữ cảnh → lệnh ẩn thực thi âm thầm, không xuất hiện trực tiếp trong câu hỏi người dùng", "Vì RAG tiêu tốn nhiều token hơn", "Vì RAG không tự cập nhật dữ liệu"], correct: 1, why: "Mã độc nằm trong tài liệu được tự động nạp vào ngữ cảnh → tấn công âm thầm, admin không thấy trong câu hỏi trực tiếp. (Phần 1)" },
  { part: A, q: "Ba lớp phòng thủ chiều sâu chống prompt injection gồm những gì?", opts: ["Điều chỉnh temperature · giới hạn token · đổi sang mô hình lớn hơn", "Cache · trim · routing", "Tách chỉ dẫn hệ thống khỏi dữ liệu đầu vào · Đặc quyền tối thiểu (Least Privilege) · Kiểm duyệt output trước khi thực thi hành động", "Xác thực input · kiểm output · lặp kiểm chứng"], correct: 2, why: "Ba lớp: tách chỉ thị khỏi dữ liệu · Least Privilege · kiểm duyệt output trước khi hành động. (Phần 1)" },
  { part: A, q: "Nhận định nào ĐÚNG nhất về phòng chống prompt injection?", opts: ["Chỉ cần một giải pháp bảo mật duy nhất là đủ an toàn tuyệt đối", "Đây là lỗi cố hữu của LLM, hoàn toàn không thể phòng ngừa", "Chỉ cần tăng temperature là ngăn được tấn công", "Không có \"viên đạn bạc\" — bắt buộc phòng thủ nhiều lớp phối hợp"], correct: 3, why: "Không có giải pháp vạn năng; phải phòng thủ chiều sâu nhiều lớp bổ trợ nhau. (Phần 1)" },
  { part: B, q: "Một eval set đạt chuẩn cần bao gồm những nhóm kịch bản nào?", opts: ["Happy path (thông thường) · edge cases (biên) · adversarial cases (tấn công đối nghịch)", "Chỉ cần các kịch bản hoạt động thông thường", "Chỉ cần các kịch bản tấn công thử nghiệm", "Chỉ cần những kịch bản đơn giản nhất để kiểm tra nhanh"], correct: 0, why: "Eval set chuẩn có đủ 3 nhóm: happy path · edge · adversarial. (Phần 2)" },
  { part: B, q: "Chạy eval trước và sau khi thay đổi mã nguồn giúp phát hiện điều gì?", opts: ["Chi phí điện năng của máy chủ", "Lỗi hồi quy (regression — suy giảm chất lượng ở tính năng vốn đang chạy đúng)", "Tông màu và bố cục giao diện", "Số lượt commit lên hệ thống quản lý phiên bản"], correct: 1, why: "Eval trước–sau giúp bắt hồi quy: cập nhật mới vô tình làm hỏng phần trước đó vốn chạy đúng. (Phần 2)" },
  { part: B, q: "Ở cấp độ L2, eval suite cho Capstone cần tối thiểu bao nhiêu kịch bản?", opts: ["Chỉ cần 1 kịch bản", "Chỉ cần 2 kịch bản", "Tối thiểu 10 kịch bản bao phủ đủ cả 3 nhóm chính", "Không bắt buộc kịch bản nào"], correct: 2, why: "Tối thiểu 10 kịch bản thuộc cả 3 nhóm để đảm bảo phát hiện hồi quy ở L2. (Phần 2)" },
  { part: B, q: "Vì sao che giấu PII thủ công chắc chắn thất bại khi vận hành ở quy mô lớn?", opts: ["Vì thủ công nhanh hơn tự động hóa", "Vì máy tính không nhận diện được ký tự PII", "Vì PII không quan trọng khi vận hành hệ thống lớn", "Vì con người dễ sai sót, không thể nhớ che PII trong hàng nghìn yêu cầu mỗi ngày → bắt buộc tự động hóa"], correct: 3, why: "Ở quy mô lớn con người không thể nhớ xử lý từng lượt → khử PII bắt buộc phải tự động. (Phần 2)" },
  { part: B, q: "Ba vị trí bắt buộc phải tự động khử PII trong hệ thống AI là gì?", opts: ["Trước khi gửi tới mô hình · trong log hệ thống · trong dữ liệu nguồn kho RAG", "Chỉ lọc trong log hệ thống", "Chỉ lọc trước khi gửi tới mô hình", "Che giấu sau khi đã phản hồi kết quả tới khách"], correct: 0, why: "Ba vị trí: trước khi tới mô hình · khi ghi log · trước khi nạp tài liệu vào kho RAG. (Phần 2)" },
  { part: B, q: "Hoạt động khử PII (PII Redaction) diện rộng nhằm tuân thủ quy định pháp luật nào tại Việt Nam?", opts: ["Không tuân theo quy định nào", "Luật số 91/2025/QH15 (hiệu lực 01/01/2026) và Nghị định 356/2025/NĐ-CP về bảo vệ dữ liệu cá nhân", "Chỉ đáp ứng quy chế nội bộ phòng ban", "Luật bảo vệ dữ liệu của các quốc gia khác"], correct: 1, why: "Tuân thủ Luật số 91/2025/QH15 và Nghị định 356/2025/NĐ-CP về bảo vệ dữ liệu cá nhân. (Phần 2, kết hợp I1.2)" },
  { part: B, q: "Một risk register tiêu chuẩn cần định nghĩa những thông tin cốt lõi nào cho mỗi rủi ro?", opts: ["Chỉ ghi tên gọi của rủi ro", "Chỉ đánh giá mức độ nghiêm trọng và tần suất", "Bản chất rủi ro → biện pháp kiểm soát → phân công cụ thể người chịu trách nhiệm", "Chỉ ghi các giải pháp lập trình kỹ thuật thuần túy"], correct: 2, why: "Risk register chuẩn: bản chất → cơ chế kiểm soát → người chịu trách nhiệm cụ thể. (Phần 2)" },
  { part: C, q: "Nguyên nhân chính của lỗi \"chạy bình thường trên máy tôi\" khi chia sẻ mã nguồn?", opts: ["Mã nguồn luôn bị viết sai logic", "Lập trình viên quên commit lên Git", "Cài temperature của mô hình quá cao", "Ứng dụng phụ thuộc cấu hình môi trường cục bộ (phiên bản thư viện, biến môi trường, HĐH) không được đóng gói & chia sẻ đồng bộ"], correct: 3, why: "Lỗi do khác biệt môi trường (thư viện, biến môi trường, HĐH) không được đóng gói kèm mã nguồn. (Phần 3)" },
  { part: C, q: "Docker giải quyết lỗi không tương thích môi trường bằng cách nào?", opts: ["Đóng gói app cùng toàn bộ cấu hình môi trường vào một container chạy đồng nhất trên mọi hạ tầng", "Tăng băng thông & tốc độ mạng của máy chủ", "Tự viết lại mã nguồn cho tương thích máy chủ", "Tự quét & che PII của khách"], correct: 0, why: "Docker đóng gói app + cấu hình môi trường vào container khép kín → chạy nhất quán mọi nơi. (Phần 3)" },
  { part: C, q: "Phân biệt Image và Container trong Docker như thế nào?", opts: ["Là hai thuật ngữ đồng nghĩa, thay thế cho nhau", "Image là khuôn mẫu thiết kế tĩnh (định nghĩa qua Dockerfile); Container là phiên bản chạy thực tế (runtime instance) tạo từ Image", "Container là khuôn mẫu tĩnh; Image là phiên bản chạy thực tế", "Cả hai đều là kiến trúc mô hình học máy"], correct: 1, why: "Image = khuôn mẫu tĩnh (Dockerfile); Container = phiên bản chạy thực tế tạo từ Image. (Phần 3)" },
  { part: C, q: "Nguyên tắc bảo mật quan trọng nhất khi đóng gói ứng dụng bằng Docker?", opts: ["Lưu trực tiếp API key / mật khẩu vào Image cho thuận tiện", "Đóng gói cơ sở dữ liệu chứa thông tin thực của người dùng vào Image", "Tuyệt đối KHÔNG lưu secret / PII vào Image; truyền secret qua biến môi trường khi khởi chạy container", "Không cần quan tâm bảo mật khi đóng gói Image"], correct: 2, why: "Không lưu secret / PII trong Image tĩnh; truyền secret qua biến môi trường khi chạy container. (Phần 3, kết hợp I4.2)" },
  { part: C, q: "Sau khi đưa vào production, người quản trị cần liên tục giám sát các chỉ số nào?", opts: ["Đã triển khai thì chạy tự động hoàn toàn, không cần giám sát", "Chỉ theo dõi màu sắc & giao diện người dùng", "Chỉ đếm số commit mã nguồn", "Chi phí token · latency · tỷ lệ lỗi (biến động bất thường cảnh báo bị tấn công / lạm dụng tài nguyên)"], correct: 3, why: "Giám sát token cost · latency · tỷ lệ lỗi; đột biến là cảnh báo sớm bị tấn công / lạm dụng. (Phần 3, kết hợp I5.1)" },
  { part: C, q: "Sản phẩm AI đạt Production-ready ở L2 khi đáp ứng trọn bộ tiêu chuẩn nào?", opts: ["Guardrails + Eval suite + tự động khử PII + Risk register + đóng gói Docker thành công + giám sát token cost", "Chỉ cần chạy ổn định không lỗi lúc demo", "Chỉ cần giao diện đồ họa đẹp & thu hút", "Chỉ cần tích hợp càng nhiều tính năng càng tốt"], correct: 0, why: "Production-ready L2 = Guardrails + Eval + khử PII + Risk register + Docker + giám sát token cost. (Phần 3)" },
];
