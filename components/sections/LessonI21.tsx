...
const PARTS = [
  { ...PART_META[0], desc: "Prompt là \"artifact\", system prompt (vai trò/format/ràng buộc), structured output để output dùng được ngay.", tags: ["Artifact", "System prompt", "Structured output"] },
  { ...PART_META[1], desc: "Zero/few-shot, CoT, task decomposition, context injection, iterate có chẩn đoán + cheat sheet.", tags: ["CoT", "Decomposition", "Iterate"] },
  { ...PART_META[2], desc: "Multimodal (text+image), tool use / function calling, API là gì, khi nào cần tool/API thay vì chỉ prompt.", tags: ["Multimodal", "Tool use", "API"] },
];

const OBJECTIVES = [
  "Thiết kế chỉ dẫn hệ thống (system prompt) có cấu trúc: xác định rõ vai trò, định dạng đầu ra và các ràng buộc kỹ thuật để kết quả đầu ra (output) ổn định.",
  "Yêu cầu đầu ra có cấu trúc (structured output) dạng bảng, JSON hoặc biểu mẫu template để kết quả dùng được ngay và dễ kiểm soát chất lượng (QC).",
  "Áp dụng linh hoạt zero-shot / few-shot / chain-of-thought, task decomposition (phân rã nhiệm vụ), context injection (truyền ngữ cảnh) cho bài toán sản phẩm thực tế.",
  "Cải tiến có chẩn đoán (iterate): khi kết quả (output) sai, xác định đúng nguyên nhân và sửa đúng chỗ trong ≤2 lần.",
  "Kết hợp xử lý đa phương thức (multimodal gồm văn bản + hình ảnh) và hiểu cách sử dụng công cụ (tool use / function calling) và giao diện kết nối API — biết rõ khi nào nên áp dụng.",
];

const MUST_KNOW = ["Prompt", "Prompt Engineering", "System Prompt", "Zero-shot/Few-shot", "Chain-of-thought", "Tool Use/Function Calling", "Multimodal"];
...
const WHEN_TABLE = [
  { sit: "Viết, tóm tắt, phân loại, brainstorm trên dữ liệu bạn dán vào", how: "Chỉ cần prompt (Phần 1–2)", c: "var(--iris-deep)" },
  { sit: "Cần dữ liệu thật/cập nhật (tồn kho, đơn hàng, doanh số)", how: "Tool use / function calling — cho AI gọi ra lấy số thật", c: "var(--gold-deep)" },
  { sit: "Cần AI chạy tự động trong một feature/sản phẩm, quy mô lớn", how: "Gọi qua API", c: "var(--mint-deep)" },
  { sit: "Cần AI dùng kho tài liệu nội bộ để trả lời", how: "Tìm kiếm tăng cường (RAG) + Giao thức bối cảnh mô hình (MCP) → học ở I3.1", c: "var(--rose-deep)" },
];
...
const EXAM: ExamQ[] = [
  ...
  { part: B, q: "Context injection gồm bốn trục nào?", opts: ["Đối tượng · ràng buộc vận hành · mục tiêu · giọng thương hiệu", "Token · temperature · cửa sổ ngữ cảnh (context window) · chi phí (cost)", "Vai trò · JSON · bảng · template", "Tên · SĐT · địa chỉ · email"], correct: 0, why: "Context injection: đối tượng · ràng buộc vận hành · mục tiêu · giọng thương hiệu." },
  ...
  { part: C, q: "Multimodal nghĩa là gì?", opts: ["Dùng nhiều mô hình AI cùng lúc", "Trả lời được nhiều ngôn ngữ", "Chạy trên nhiều máy chủ", "Mô hình xử lý được nhiều loại đầu vào — phổ biến nhất là văn bản + hình ảnh (multimodal)"], correct: 3, why: "Multimodal = xử lý nhiều loại đầu vào, phổ biến nhất là văn bản + hình ảnh (multimodal)." },
  ...
  { part: C, q: "Tool Use / Function Calling giúp gì?", opts: ["Cho AI gọi công cụ/hàm để lấy dữ liệu thật thay vì bịa → giảm lỗi ảo tưởng (hallucination) số liệu", "Làm AI viết văn hay hơn", "Cửa sổ ngữ cảnh (context window) lớn hơn", "Luôn luôn giảm chi phí token"], correct: 0, why: "Tool use/function calling cho AI gọi công cụ lấy số thật → giảm lỗi ảo tưởng (hallucination) số liệu." },
  { part: C, q: "API là gì trong việc build một feature AI?", opts: ["Một loại prompt đặc biệt", "Một mô hình AI mới", "Cổng kết nối (API) giúp phần mềm gọi mô hình tự động (gửi prompt, nhận kết quả), chi phí được tính theo số lượng token", "Một bộ lọc PII"], correct: 2, why: "API = cổng kết nối giúp phần mềm gọi mô hình tự động, chi phí tính theo token." },
...
function Part1View() {
  return (
    <div>
      ...
      <p style={{ font: "400 18px/1.8 var(--font-body)", color: "var(--fg-2)", margin: "0 0 20px" }}><b>Structured Output</b> là yêu cầu AI trả về kết quả theo đúng <b>schema</b> (cấu trúc dữ liệu định sẵn) thay vị để AI tự do trình bày. Ba dạng schema hay dùng:</p>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "12px", margin: "0 0 24px" }}>
        ...
        <div style={{ padding: "16px 18px", border: "1px solid var(--mint)", borderRadius: "12px", background: "var(--mint-tint)" }}><div style={{ font: "700 13px/1 var(--font-brand)", color: "var(--mint-deep)", marginBottom: "6px" }}>Template điền mục</div><div style={{ font: "13px/1.55 var(--font-body)", color: "var(--fg-1)" }}>Khi output là tài liệu (brief/report/spec) và cần đủ mọi mục.</div></div>
      </div>
      ...
      <div style={{ margin: "24px 0", padding: "18px 22px", background: "var(--gold-tint)", borderLeft: "3px solid var(--gold)", borderRadius: "0 10px 10px 0" }}>
        ...
        <p style={{ font: "16px/1.7 var(--font-body)", color: "var(--fg-1)", margin: 0 }}>Thay vì hỏi "phân bổ 500 áo khoác cho 5 cửa hàng và giải thích" (nhận về 6 đoạn văn phải đọc để nhặt số), hãy yêu cầu: <i>"Trả về đúng bảng sau, không thêm lời dẫn: | Cửa hàng | Nhu cầu ròng | Phân bổ cuối | Lý do (1 câu) |"</i> → nhận về bảng sạch, kiểm tra tổng cột "Phân bổ cuối" xem có bằng 500 hay không chỉ mất 10 giây.</p>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "10px", margin: "0 0 26px" }}>
        {[
          ...
          "Nếu cần tính toán trung gian, <b>kết hợp với chain-of-thought</b> (học ở Phần 2): \"trình bày tính toán, sau đó trả về bảng theo schema…\".",
      ...
      <TldrDark items={[
        ...
        "<b>Structured output</b> (bảng/JSON/template) làm kết quả dùng được ngay và dễ kiểm soát chất lượng (QC); khai báo schema trước, yêu cầu lược bỏ hoàn toàn lời dẫn thừa.",
      ]} />
  ...
  return (
    <div>
      ...
      <div style={{ margin: "0 0 26px", padding: "16px 20px", border: "1px solid var(--rose-deep)", borderRadius: "12px", background: "var(--rose-tint)", display: "flex", gap: "14px", alignItems: "flex-start" }}>
        ...
        <p style={{ font: "15px/1.65 var(--font-body)", color: "var(--fg-1)", margin: 0 }}><b>Bẫy:</b> CoT mượt mà <i>không</i> đồng nghĩa CoT đúng. AI có thể sai một bước mà kết luận vẫn "nghe hợp lý". <b>Quy tắc L2: Phải kiểm tra chuỗi suy luận chứ không chỉ nhìn vào đáp số cuối cùng — đặc biệt là các bước có tính toán số liệu.</b> (Lưu ý: CoT sẽ tiêu tốn nhiều token hơn và tăng độ trễ phản hồi, vì vậy chỉ áp dụng khi thực sự cần thiết).</p>
      </div>
      ...
      <p style={{ font: "400 18px/1.8 var(--font-body)", color: "var(--fg-2)", margin: "0 0 16px" }}>Khi một việc gồm nhiều giai đoạn <i>khác bản chất</i> (trích xuất → gom nhóm → suy luận → xếp hạng), nếu nhồi nhét tất cả yêu cầu vào một prompt duy nhất, AI rất dễ làm việc sơ sài, bỏ sót thông tin (đặc biệt là các yêu cầu nằm ở giữa) hoặc làm lẫn lộn các phần với nhau. <b>Task decomposition</b> tách thành chuỗi prompt nối tiếp, mỗi bước có input/output rõ; output bước trước là input bước sau.</p>
      <p style={{ font: "400 16px/1.7 var(--font-body)", color: "var(--fg-2)", margin: "0 0 20px" }}><b style={{ color: "var(--fg-1)" }}>Lợi ích:</b> kiểm từng bước riêng (sai Bước 1 thì sửa Bước 1, tránh việc phải chạy lại toàn bộ quy trình từ đầu), tái sử dụng các kết quả trung gian, cải thiện từng phần. <b style={{ color: "var(--fg-1)" }}>Nguyên tắc:</b> phân rã <i>đủ để kiểm soát</i>, tránh chia quá nhỏ hoặc quá vụn vặt. Tự hỏi "nếu bước này sai, tôi có cần biết riêng không?" Nếu chỉ cần kết quả cuối → không tách.</p>
      ...
      <p style={{ font: "400 18px/1.8 var(--font-body)", color: "var(--fg-2)", margin: "0 0 18px" }}>Khi hỏi mà không cấp ngữ cảnh, AI trả lời "phù hợp với tất cả mọi người" — tức không đặc biệt hợp với ai, nghe rập khuôn như sách giáo khoa. <b>Context injection</b> là chủ động chèn bối cảnh trước khi đặt yêu cầu, theo bốn trục:</p>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", margin: "0 0 20px" }}>
        ...
        <div style={{ border: "1px solid var(--border)", borderRadius: "12px", background: "#fff", padding: "15px 18px" }}><b style={{ font: "700 14px/1.3 var(--font-brand)", color: "var(--rose-deep)" }}>Giọng điệu/thương hiệu (tone of voice)</b><div style={{ font: "14px/1.6 var(--font-body)", color: "var(--fg-2)", marginTop: "3px" }}>Tone phù hợp (YODY: trẻ trung, thực dụng, không hoa mỹ).</div></div>
      </div>
      <div style={{ margin: "0 0 26px", padding: "18px 22px", background: "var(--iris-tint)", borderLeft: "3px solid var(--iris)", borderRadius: "0 10px 10px 0" }}>
        ...
        <p style={{ font: "16px/1.7 var(--font-body)", color: "var(--fg-1)", margin: 0 }}>"Viết mô tả áo khoác gió" → ra văn sáo rỗng. Thêm context (<i>"khách đi xe máy đô thị 22–30 tuổi · ≤60 chữ · không dùng 'cao cấp' · mô tả trên app, đọc 5 giây quyết định"</i>) → tạo ra mô tả sắc bén, sát với thực tế sản phẩm và đúng giọng điệu thương hiệu. Nhớ từ I1.1: context injection chính là cách bạn <b>cấp sự thật cụ thể của mình</b> để AI khỏi phải đoán.</p>
      </div>
      ...
      <p style={{ font: "400 18px/1.8 var(--font-body)", color: "var(--fg-2)", margin: "0 0 20px" }}>Khi kết quả (output) sai, người mới bắt đầu thường chỉnh sửa prompt theo cảm tính (sửa mò). Ngược lại, người xây dựng cấp độ L2 sẽ tiến hành chẩn đoán nguyên nhân trước khi sửa để tác động đúng chỗ. Tiêu chuẩn của L2 là đạt được kết quả mong muốn chỉ sau tối đa 2 lần tinh chỉnh.</p>
      ...
      <TldrDark items={[
        "<b>Zero/few-shot, CoT, task decomposition, context injection</b> là bốn công cụ điều khiển AI; chúng ghép được vào nhau trong một pipeline nối tiếp (pipeline).",
...
function Part3View({ go }: { go: (p: Page, part?: number) => void }) {
  return (
    <div>
      ...
      <p style={{ font: "400 18px/1.8 var(--font-body)", color: "var(--fg-2)", margin: "0 0 18px" }}><b>Multimodal</b> nghĩa là mô hình xử lý được nhiều loại dữ liệu đầu vào, không chỉ văn bản — phổ biến nhất đối với người xây dựng (builder) là <b>văn bản + hình ảnh (multimodal)</b>. Bạn có thể đưa ảnh kèm câu hỏi, và AI "đọc" được nội dung ảnh. Ứng dụng ở YODY (giả lập):</p>
      <div style={{ display: "flex", flexDirection: "column", gap: "10px", margin: "0 0 20px" }}>
        ...
        <div style={{ display: "flex", gap: "12px", alignItems: "flex-start", border: "1px solid var(--border)", borderRadius: "10px", background: "#fff", padding: "14px 16px" }}><span style={{ color: "var(--rose)", flex: "none", fontSize: "17px" }}>📋</span><span style={{ font: "14px/1.6 var(--font-body)", color: "var(--fg-1)" }}>Đưa <b>ảnh chụp bảng size / tem sản phẩm</b> để AI trích thông tin ra dạng bảng.</span></div>
        <div style={{ display: "flex", gap: "12px", alignItems: "flex-start", border: "1px solid var(--border)", borderRadius: "10px", background: "#fff", padding: "14px 16px" }}><span style={{ color: "var(--rose)", flex: "none", fontSize: "17px" }}>💬</span><span style={{ font: "14px/1.6 var(--font-body)", color: "var(--fg-1)" }}>Đưa <b>ảnh chụp màn hình đánh giá (screenshot review)</b> để phân loại (nhưng nhớ bảo mật thông tin cá nhân (PII) trong ảnh trước — nhắc lại I1.2).</span></div>
      </div>
      <div style={{ margin: "0 0 26px", padding: "16px 20px", background: "var(--rose-tint)", borderLeft: "3px solid var(--rose-deep)", borderRadius: "0 10px 10px 0", font: "15px/1.7 var(--font-body)", color: "var(--fg-1)" }}><b style={{ color: "var(--rose-deep)" }}>Vẫn áp dụng mọi nguyên tắc đã học:</b> structured output (bắt AI trả bảng), grounding (chỉ mô tả cái thấy trong ảnh, không bịa thông số không nhìn thấy), và ràng buộc định dạng. Multimodal mở rộng <i>đầu vào (inputs)</i>, không thay đổi nguyên tắc thiết kế prompt.</div>
      ...
      <p style={{ font: "400 18px/1.8 var(--font-body)", color: "var(--fg-2)", margin: "0 0 18px" }}>Bản thân LLM chỉ có khả năng dự đoán và xử lý ngôn ngữ — nó không tự nắm bắt được doanh số hôm nay, không tra cứu được lượng tồn kho thực tế, cũng không thể tự động gửi email. <b>Sử dụng công cụ (Tool Use) hay Gọi hàm (Function Calling)</b> là cơ chế cho phép mô hình <b>yêu cầu chạy một công cụ hoặc hàm lập trình bên ngoài</b> để lấy dữ liệu thực tế hoặc thực hiện một hành động cụ thể, sau đó sử dụng kết quả đó để hoàn thiện câu trả lời.</p>
      <div style={{ display: "flex", flexDirection: "column", gap: "10px", margin: "0 0 20px" }}>
        <div style={{ display: "flex", gap: "14px", alignItems: "flex-start", border: "1px solid var(--border)", borderRadius: "12px", background: "#fff", padding: "15px 18px" }}><span style={{ font: "700 12px/1 var(--font-mono)", color: "#fff", background: "var(--iris)", padding: "6px 10px", borderRadius: "6px", flex: "none" }}>1</span><div style={{ font: "14px/1.6 var(--font-body)", color: "var(--fg-1)" }}>Bạn khai báo cho AI biết có những "tool" nào (ví dụ: <span style={{ fontFamily: "var(--font-mono)" }}>tra_ton_kho(mã_SP)</span>, <span style={{ fontFamily: "var(--font-mono)" }}>tra_doanh_so(cửa_hàng, tháng)</span>).</div></div>
      ...
      <p style={{ font: "400 18px/1.8 var(--font-body)", color: "var(--fg-2)", margin: "0 0 18px" }}><b>API (Application Programming Interface)</b> là giao diện lập trình ứng dụng, đóng vai trò làm cổng kết nối để phần mềm của bạn tương tác với mô hình AI một cách tự động, thay vì phải nhập liệu thủ công trên giao diện chat. Khi bạn build một tính năng AI (feature) thực tế (chatbot, tự sinh mô tả hàng loạt…), tính năng đó <b>gọi API</b> của mô hình: gửi prompt vào, nhận kết quả ra.</p>
      <div style={{ display: "flex", flexDirection: "column", gap: "10px", margin: "0 0 26px" }}>
        <div style={{ display: "flex", gap: "12px", alignItems: "flex-start", border: "1px solid var(--border)", borderRadius: "10px", background: "#fff", padding: "13px 16px" }}><span style={{ color: "var(--iris)", flex: "none", fontSize: "16px" }}>💰</span><span style={{ font: "14px/1.6 var(--font-body)", color: "var(--fg-1)" }}>Gọi API là nơi các khái niệm ở I1.1 thành <i>tiền thật</i>: bạn chi trả dựa trên <b>số lượng token tiêu thụ</b> (input + output), chọn <b>mô hình (model)</b> theo cân đối tốc độ/chi phí/độ chính xác (đào sâu ở I5.1).</span></div>
        ...
        <div style={{ display: "flex", gap: "12px", alignItems: "flex-start", border: "1px solid var(--border)", borderRadius: "10px", background: "#fff", padding: "13px 16px" }}><span style={{ color: "var(--mint-deep)", flex: "none", fontSize: "16px" }}>🔌</span><span style={{ font: "14px/1.6 var(--font-body)", color: "var(--fg-1)" }}><b>Giao thức bối cảnh mô hình (Model Context Protocol - MCP)</b> là một chuẩn kết nối AI với công cụ/nguồn dữ liệu doanh nghiệp — bạn chỉ cần <b>biết tên</b> ở đây; cơ chế và thực hành sẽ học sâu ở **I3.1**.</span></div>
      </div>
      ...
      <TldrDark items={[
        "<b>Multimodal</b> mở rộng đầu vào (văn bản + hình ảnh) nhưng vẫn giữ nguyên tắc thiết kế prompt (structured output, grounding, bảo mật thông tin cá nhân PII trong ảnh).",
        "<b>Tool Use / Function Calling</b> cho AI gọi công cụ lấy <b>dữ liệu thật</b> — giải pháp triệt để cho lỗi ảo tưởng (hallucination) số liệu.",
        "<b>API</b> là phương thức kết nối tự động giữa ứng dụng và mô hình AI (chi trả dựa trên lượng token); MCP chỉ nhắc tên, học sâu ở I3.1.",
      ]} />
