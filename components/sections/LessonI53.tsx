...
const PART_META = [
  { n: "01", short: "Ship vs Demo & Tài liệu", title: "Ship vs Demo, Tài liệu kiến trúc & Integrity", time: "~15 phút", c: "var(--iris)", cDeep: "var(--iris-deep)", tint: "var(--iris-tint)" },
...
const PARTS = [
  { ...PART_META[0], desc: "Ship ≠ Demo (người dùng thật + impact đo được), bốn nguyên tắc tốt nghiệp với điều kiện cứng integrity at scale, và tài liệu kiến trúc & rủi ro như một phần bắt buộc của sản phẩm.", tags: ["Ship ≠ Demo", "4 nguyên tắc tốt nghiệp", "Tài liệu kiến trúc", "Integrity at scale"] },
  { ...PART_META[1], desc: "Cấu trúc buổi bảo vệ Demo Day 3 phần, bảo vệ = chịu trách nhiệm không trình diễn, 6 câu hỏi phản biện thường gặp & cách trả lời neo về trách nhiệm, cùng demo dự phòng (fallback).", tags: ["Cấu trúc Demo Day", "Bảo vệ = chịu trách nhiệm", "6 câu phản biện", "Demo dự phòng"] },
];

const OBJECTIVES = [
  "Phân biệt triển khai thực tế (ship) và chạy thử (demo); trình bày sản phẩm đã đưa vào sử dụng thực tế với outcome đo lường được.",
  "Trình bày tài liệu kiến trúc & rủi ro như một phần của sản phẩm.",
  "Bảo vệ = chịu trách nhiệm: trả lời được &quot;nếu hỏng thì sao&quot;, &quot;ai chịu khi lộ dữ liệu&quot;, &quot;vì sao chọn thiết kế này&quot;.",
  "Hiểu integrity at scale là điều kiện bắt buộc — để lộ PII không kiểm soát thì vẫn KHÔNG ĐẠT.",
  "Nắm chuẩn tốt nghiệp Product Builder (L2) và chuẩn bị đủ bằng chứng.",
];
...
const EXAM: ExamQ[] = [
  ...
  { part: B, q: "Nguyên tắc đúng khi bảo vệ trước hội đồng là gì?", opts: ["Giả vờ sản phẩm hoàn hảo, giấu giới hạn", "Kể càng nhiều công sức càng tốt", "Trình bày bằng chứng thực tế, trung thực về giới hạn, và gắn trách nhiệm với quyết định đánh đổi (&quot;ai trả giá / ai chịu trách nhiệm&quot;)", "Tránh nói về rủi ro"], correct: 2, why: "Kể bằng chứng thực tế, trung thực về giới hạn, neo về trách nhiệm (&quot;ai trả giá/ai chịu&quot;). (File 2)" },
  { part: B, q: "Hội đồng hỏi &quot;Nếu sản phẩm hỏng thì sao?&quot; — nên trả lời dựa trên?", opts: ["&quot;Sản phẩm em không bao giờ hỏng&quot;", "Đổ lỗi cho AI", "Xin thêm thời gian", "Trình bày phương án dự phòng (fallback): khi AI trả về kết quả sai, rỗng hoặc quá thời gian (timeout) thì người dùng thấy gì, đảm bảo không lỗi ngầm (vỡ âm thầm)"], correct: 3, why: "&quot;Nếu hỏng&quot; → trình bày fallback: khi AI sai/rỗng/timeout thì người dùng thấy gì để không lỗi ngầm. (File 2)" },
  ...
  { part: B, q: "Với câu phản biện bạn CHƯA xử lý được, cách tốt nhất là gì?", opts: ["Chống chế cho qua", "Nói thẳng &quot;đây là giới hạn hiện tại, em sẽ xử lý bằng X&quot; — thể hiện biết mình thiếu gì", "Im lặng", "Đổi sang chủ đề khác"], correct: 1, why: "Nói thẳng giới hạn hiện tại + kế hoạch cải tiến, không chống chế. (File 2)" },
  { part: C, q: "Bốn tiêu chí bắt buộc (must-pass) để tốt nghiệp là gì?", opts: ["NL4 · NL5 · NL6 · NL7", "Tất cả 7 năng lực ở L3", "Năng lực 5 (NL5 - Prompt & Tool) · Năng lực 1 (NL1 - Ownership) · Năng lực 3 (NL3 - Cộng tác) · Năng lực 7 (NL7 - Bảo vệ PII)", "Chỉ cần NL5"], correct: 2, why: "4 must-pass: NL5 · NL1 · NL3 · NL7 (PII). (File 3)" },
  { part: C, q: "Ngoài 4 must-pass, điều kiện điểm tổng để tốt nghiệp là gì?", opts: ["Đạt L3 cả 7 năng lực", "Chỉ cần đạt 4 must-pass", "Đạt L5 Architect", "Đạt tối thiểu L2 ở cả 7/7 năng lực"], correct: 3, why: "Điểm tổng: đạt tối thiểu L2 ở cả 7/7 năng lực. (File 3)" },
  { part: C, q: "Mục đầu tiên trong checklist bằng chứng tốt nghiệp là gì?", opts: ["Sản phẩm AI cuối khóa đã SHIP cho người dùng thật (không phải chỉ demo)", "Slide thật đẹp", "Có thật nhiều tính năng", "Code thật dài"], correct: 0, why: "Checklist đầu tiên: sản phẩm đã SHIP cho người dùng thật. (File 3)" },
  { part: C, q: "Rubric hội đồng gồm 4 trục nào?", opts: ["Token · Cost · Latency · Accuracy", "Bằng chứng người dùng thật · Độ đầy đủ tài liệu · Cơ chế kiểm soát chạy thật · Chất lượng bảo vệ", "Problem · User · Metric · Risk", "Git · Test · Deploy · Log"], correct: 1, why: "Rubric 4 trục: bằng chứng người dùng · độ đầy đủ tài liệu · cơ chế kiểm soát chạy thật · chất lượng bảo vệ. (File 3)" },
  { part: C, q: "&quot;Stretch&quot; (không bắt buộc) khi tốt nghiệp là gì?", opts: ["Đạt L2 cả 7 năng lực", "Ship được sản phẩm", "Đạt L3 ở 1–2 năng lực thế mạnh → cân nhắc thăng tiến sớm (Product Engineer)", "Làm nhiều tính năng nhất lớp"], correct: 2, why: "Stretch: đạt L3 ở 1–2 năng lực thế mạnh → cơ sở thăng tiến sớm (Product Engineer). (File 3)" },
  { part: C, q: "Sau khi tốt nghiệp Product Builder (L2), chặng tiếp theo trên career ladder là gì?", opts: ["Nghỉ chương trình", "Quay lại L1", "Principal / Staff", "Product Engineer (L3) — làm chủ toàn trình (end-to-end) một tính năng với sự tự chủ hoàn toàn"], correct: 3, why: "Chặng tiếp theo: Product Engineer (L3) — làm chủ end-to-end, tự chủ hoàn toàn. (File 3)" },
];
